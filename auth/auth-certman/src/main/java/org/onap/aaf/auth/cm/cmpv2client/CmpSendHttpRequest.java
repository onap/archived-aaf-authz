...
public class CmpSendHttpRequest {
    ...
    private static final String CMP_CONTENT_TYPE_VALUE = "application/pkixcmp";

    public CmpSendHttpRequest(CloseableHttpClient httpClient) {
     ...
     */
    public PKIMessage cmpSendHttpPostRequest(final PKIMessage pkiMessage, final String urlString)
        throws CmpClientException, CAOfflineException {

        HttpPost postRequest = new HttpPost(urlString);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try {
            byte[] requestBytes = pkiMessage.getEncoded();
            postRequest.setEntity(new ByteArrayEntity(requestBytes));
            postRequest.setHeader(CONTENT_TYPE, CMP_CONTENT_TYPE_VALUE);
        ...
        return PKIMessage.getInstance(baos.toByteArray());
    }
