    ...
    public int _exec(final int idx, final String ... args) throws CadiException, APIException, LocatorException {
        ...
        return same(new Retryable<Integer>() {
            ...
            public Integer code(Rcli<?> client) throws CadiException, APIException {    
                ...
                switch(option) {
                    case 0: 
                        fp = client.create(authz+ns+atrributeString+key+'/'+value,Void.class);
                        message = String.format("Add Attrib %s=%s to %s",
                                key,value,ns);
                        break;
                    case 1: 
                        fp = client.update(authz+ns+atrributeString+key+'/'+value);
                        message = String.format("Update Attrib %s=%s for %s",
                                key,value,ns);
                        break;
                    case 2: 
                        fp = client.delete(authz+ns+atrributeString+key,Void.class);
